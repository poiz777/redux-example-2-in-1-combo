      ...
      let rayTodoItems = this.props.todo.map((todoItem, iKey) => {
        ...
        return (todoItem.id === undefined) ? null : (
          <li className={ `list-group-item todo-list-item ${doneTaskClass}`} key={todoItem.id}>
            <span className="pz-task pz-item-inner pull-left">
              <span className="todo-date"><span className="due-for">Due:</span> {todoItem.date}</span>
              <br />
              <span className="badge badge-dark">{iKey+1}</span>
              <span className="todo-info">{todoItem.todo}</span>
            </span>
            <span className="pz-actions pz-item-inner pull-right">
...
// <DateTimePicker nChange={this.onChange} value={this.now}  />
// <Editor apiKey={this.apiKey} id="todoText" init={{  /* your other settings */ }} ref={this.todoField}  onKeyDown={e=>{ this.addToDoItem(e)}} />